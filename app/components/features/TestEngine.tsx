...
export default function TestEngine({ testId, category }: { testId: string, category: string }) {
  ...
  useEffect(() => {
    ...
    if (config.engine === 'REACTION') {
      ...
      if (currentRound.isTarget === false) {
         const autoPass = setTimeout(() => {
            if (gameState === 'playing' && !reactionStartTime) { 
               handleAnswer(0, true);
            }
         }, (currentRound.timeLimit || 3) * 1000 + delay);
         return () => { clearTimeout(t); clearTimeout(autoPass); };
      }
    ...
    if (config.engine === 'AI_CAMERA') {
      ...
      const t = setInterval(() => {
        ...
        if (p >= 100) {
          clearInterval(t);
          const isFailed = tremorScore > 400 || (gazeMetrics.jitter > 0.4); 
  ...
  }, [gameState, round, config, tremorScore, gazeMetrics.jitter]);
  ...
  const handleAnswer = (choice: any, customCorrect?: boolean) => {
    ...
    setTimeout(() => {
      ...
      if (round + 1 < config.rounds.length) {
        ...
        const pct = Math.round((finalScore / config.rounds.length) * 100);
        ...
        const session: TestSession = {
          ...
          rounds: roundDetails.current,
          attention: { 
            tabSwitchCount: tabSwitchCount.current, 
            inactivityCount: inactivityCount.current, 
            totalTestDurationMs: Date.now() - testStartTime.current 
          },
          emotional: { 
            frustrationEvents: frustrationEvents.current, 
            impulsivityEvents: impulsivityEvents.current 
          },
          rawScore: pct,
          postAnalysis: null, // Calculated below
          completedAt: new Date().toISOString()
        };

        // Standardized Clinical Analysis (Phase 3)
        ...
        <AnimatePresence mode="wait">
          ...
          {gameState === 'lobby' && (
            <motion.div key="lobby" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center">
              ...
              <div className="text-lg text-slate-300 font-medium mb-10 bg-slate-800/60 p-8 rounded-3xl border border-slate-700/50">{config.instructions}</div>
          ...
          {gameState === 'playing' && (
            <motion.div key="playing" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="text-center w-full">
              ...
              {config.engine === 'QUIZ' && currentRound && (
                <div>
                  <h2 className="text-3xl font-black text-white leading-tight mb-10">{currentRound.prompt}</h2>
              ...
              {config.engine === 'REACTION' && currentRound && (
                <div className="py-20 flex flex-col items-center justify-center">
                  <h3 className="text-3xl font-black text-cyan-400 mb-6 uppercase tracking-wider">{currentRound.prompt}</h3>
                  <motion.button 
                    animate={reactionTargetVisible ? { scale: [1, 1.2, 1] } : {}}
                    onClick={handleReactionClick} disabled={!!feedback}
                    className={`w-64 h-64 rounded-full font-black text-4xl text-white shadow-2xl border-8 ${reactionTargetVisible ? 'bg-emerald-500 border-emerald-300' : 'bg-red-500 border-red-800'}`}
                  >
                    {reactionTargetVisible ? '⚡ الآن!' : '...'}
                  </motion.button>
                </div>
              )}
              ...
              {config.engine === 'MEMORY' && currentRound && (
                <div>
                   <h3 className="text-3xl text-cyan-400 font-black mb-4">{currentRound.prompt}</h3>
              ...
              {config.engine === 'CANVAS' && currentRound && (
                <div className="flex flex-col items-center">
                  <h3 className="text-3xl text-cyan-400 font-black mb-6">{currentRound.prompt}</h3>
              ...
              {config.engine === 'AI_CAMERA' && currentRound && (
                <div className="py-8 flex flex-col items-center">
                   ...
                   <div className="relative w-full max-w-xl aspect-video bg-black rounded-[3rem] border-4 border-slate-800 overflow-hidden shadow-2xl">
                      {!cameraAccess && (
                        <div className="absolute inset-0 z-50 bg-slate-950/90 flex flex-col items-center justify-center p-8 text-center backdrop-blur-md">
                           <div className="text-5xl mb-4">📷</div>
                           <h3 className="text-xl font-black text-rose-500 mb-2">مطلوب إذن الكاميرا</h3>
                           <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                             من فضلكم، اسمحوا بالوصول للكاميرا ليتمكن البطل من إكمال التقييم بنجاح.
                           </p>
                           <button onClick={startCamera} className="bg-cyan-600 px-8 py-3 rounded-2xl font-bold hover:bg-cyan-500 transition-all">تفعيل الآن 🛡️</button>
                        </div>
                      )}
          ...
          {gameState === 'result' && (
            <motion.div key="result" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center p-8">
              ...
              <div className="text-[6rem] font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-500 my-8 drop-shadow-xl">{Math.round((score / config.rounds.length) * 100)}%</div>
