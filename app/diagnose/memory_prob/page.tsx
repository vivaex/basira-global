...
const MEMORY_TESTS = [
  { 
    id: 'digit-span', 
    title: 'سلسلة الأرقام (Digit Span Forward)', 
    desc: 'اختبار سعة الذاكرة العاملة اللفظية (الترتيب المباشر).', 
    icon: '🔢', 
    color: 'emerald',
    criteria: 'WISC-V: Forward Span'
  },
  { 
    id: 'digit-span-backward', 
    title: 'سلسلة الأرقام العكسية (Backward)', 
    desc: 'اختبار مرونة المعالجة الذهنية (الترتيب العكسي).', 
    icon: '🔄', 
    color: 'rose',
    criteria: 'WISC-V: Backward Span'
  },
  { 
    id: 'sequence', 
    title: 'المسار العصبي (Neural Path)', 
    desc: 'اختبار الذاكرة البصرية المكانية (مصفوفة كورسي).', 
    icon: '🧠', 
    color: 'indigo',
    criteria: 'WISC-V: Visual-Spatial Span'
  },
];
          ...
          <header className="text-center mb-20">
             ...
             <div className="flex flex-col items-center gap-4 mb-4">
                <h1 className="text-5xl md:text-7xl font-black italic">
                   <span className="text-indigo-500">مختبر</span> الذاكرة
                </h1>
                <SpeakButton 
                  text={language === 'ar' 
                    ? 'مختبر الذاكرة. تقييم الذاكرة العاملة والقدرة على معالجة المعلومات السريعة.' 
                    : 'Memory Hub. Assessing working memory and processing speed.'} 
                  size="sm" 
                />
             </div>
             <p className="text-xl text-slate-400 font-medium max-w-2xl mx-auto">
                {language === 'ar' 
                  ? 'تقييم الذاكرة العاملة والقدرة على معالجة المعلومات السريعة.' 
                  : 'Assessing working memory and processing speed.'}
             </p>
          </header>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             {MEMORY_TESTS.map((test, i) => (
                <Link key={test.id} href={`/diagnose/memory_prob/${test.id}`} className="no-underline">
                   <motion.div
