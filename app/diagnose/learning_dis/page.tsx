...
const LD_TESTS = [
  { 
    ...
    desc: 'اختبار الوعي الصوتي والقدرة على مطابقة الرموز والأصوات.', 
    icon: '🔮', 
    color: 'emerald',
    criteria: 'Phonological Awareness'
  },
  { 
    id: 'rapid-naming', 
    title: 'التسمية السريعة (Rapid Naming)', 
    desc: 'اختبار الطلاقة في تسمية الرموز والألوان والأرقام (RAN).', 
    icon: '🗯️', 
    color: 'amber',
    criteria: 'Processing Speed (RAN)'
  },
  { 
    id: 'social-recognition', 
    title: 'تمييز المشاعر (AI Robot)', 
    desc: 'اكتشف مشاعر الروبوت بصير من تعابير وجهه!', 
    icon: '🤖', 
    color: 'indigo',
    criteria: 'Social Pragmatics'
  },
  { 
    id: 'empathy-scenarios', 
    title: 'مواقف التعاطف', 
    desc: 'اقرأ الموقف واختر الشعور والتصرف المناسب.', 
    icon: '🤝', 
    color: 'indigo',
    criteria: 'Emotional Reciprocity'
  },
];

export default function LDHub() {
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-white p-6 md:p-12 relative overflow-hidden" dir={language === 'ar' ? 'rtl' : 'ltr'}>
       ...
       <div className="relative z-10 max-w-5xl mx-auto">
          <Link href="/diagnose">
            <NeonButton size="sm" color="emerald" className="mb-12">
          ...
          <header className="text-center mb-20">
             ...
             <div className="flex flex-col items-center gap-4 mb-4">
                <h1 className="text-5xl md:text-7xl font-black italic">
                   <span className="text-emerald-500">مختبر</span> صعوبات التعلم
                </h1>
                <SpeakButton 
                  text={language === 'ar' 
                    ? 'مختبر صعوبات التعلم. تقييم عيادي شامل للجوانب اللغوية والمهارات التحليلية.' 
                    : 'Learning Disabilities Hub. A comprehensive clinical assessment of linguistic and analytical skills.'} 
                  size="sm" 
                />
             </div>
             <p className="text-xl text-slate-400 font-medium max-w-2xl mx-auto">
                {language === 'ar' 
                  ? 'تقييم عيادي شامل للجوانب اللغوية والمهارات التحليلية للبطل.' 
                  : 'A comprehensive clinical assessment of linguistic and analytical skills.'}
             </p>
          </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             {LD_TESTS.map((test, i) => (
                <Link 
                  key={test.id} 
                  href={
                    test.id === 'rapid-naming' ? '/diagnose/rapid-naming' : 
                    (test.id === 'social-recognition' || test.id === 'empathy-scenarios') 
                      ? `/diagnose/social/${test.id}` 
                      : `/diagnose/learning_dis/${test.id}`
                  } 
