...
const SOCIAL_COMM_TESTS = [
  { 
    id: 'navigator', 
    title: 'ملاح القصص (Story Navigator)', 
    desc: 'اختبار الاستدلال الاجتماعي وفهم سياق المحادثة والقواعد البراغماتية.', 
    icon: '📖', 
    color: 'indigo',
    criteria: 'Pragmatic Language'
  },
  { 
    id: 'tone', 
    title: 'نبرة الصوت (Tone Recognition)', 
    desc: 'اختبار فهم النبرة، السخرية، والمعاني غير المباشرة في الكلام.', 
    icon: '🎙️', 
    color: 'rose',
    criteria: 'Inferential Thinking'
  },
];

export default function SocialCommHub() {
  const { t, language } = useLanguage();

  return (
    <main className="min-h-screen bg-[#020617] text-white p-6 md:p-12 relative overflow-hidden" dir={language === 'ar' ? 'rtl' : 'ltr'}>
       <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[70%] h-[30%] bg-indigo-600/10 rounded-full blur-[150px] animate-pulse" />
          ...
          <header className="text-center mb-20">
             ...
             <div className="flex flex-col items-center gap-4 mb-4">
                <h1 className="text-5xl md:text-7xl font-black italic">
                   <span className="text-indigo-500">مختبر</span> التواصل الاجتماعي
                </h1>
                <SpeakButton 
                  text={language === 'ar' 
                    ? 'مختبر التواصل الاجتماعي. تقييم كفاءة التواصل اللغوي والاجتماعي وفهم التلميحات غير اللفظية.' 
                    : 'Social Communication Hub. Assessing language-social competence and non-verbal cues.'} 
                  size="sm" 
                />
             </div>
             <p className="text-xl text-slate-400 font-medium max-w-2xl mx-auto">
                {language === 'ar' 
                  ? 'تقييم كفاءة التواصل اللغوي والاجتماعي وفهم التلميحات غير اللفظية.' 
                  : 'Assessing language-social competence and non-verbal cues.'}
             </p>
          </header>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             {SOCIAL_COMM_TESTS.map((test, i) => (
                <Link key={test.id} href={`/diagnose/social_comm/${test.id}`} className="no-underline">
                   <motion.div
