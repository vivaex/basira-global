...
export default function VoiceNamingLab() {
  ...
  const finishTest = (finalResults: any[]) => {
    ...
    saveTestSession({
      ...
      rawScore: totalScore,
      notes: `Voice evaluation. Transcripts: ${finalResults.map(r => `${r.target}->${r.spoken}`).join(', ')}`
    });
  };
      ...
      <div className="relative z-10 max-w-4xl mx-auto pt-12">
        <AnimatedWrapper variant="playful">
          ...
          <header className="text-center mb-12">
            <h1 className="text-4xl font-black italic tracking-tighter text-cyan-400 mb-2">
            ...
            {gameState === 'finished' && (
              <motion.div key="finished" initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }}>
                <GlassCard variant="playful" color="emerald" className="text-center p-12">
                   ...
                   <NeonButton color="emerald" onClick={() => router.push('/diagnose')}>
                     العودة للمختبرات 🗺️
                   </NeonButton>
                </GlassCard>
              </motion.div>
